...
namespace RidiculousGaming.GarageBandIdle.Tests
{
    ...
    public class ChapterImporterTests
    {
            ...
            ""modifiers"": [
                {
                    ...
                    ""appliesWhen"": {
                        ""type"": ""Any"",
                        ""conditions"": [
                            { ""type"": ""HasEntitlement"", ""entitlementId"": ""backstage_pass"" },
                            { ""type"": ""BuffActive"", ""modifier"": ""encore"" }
                        ]
                    },
                    ""effects"": [{ ""stat"": ""game_speed"", ""multiplier"": 2 }]
                }
            ],
            ""permanentModifiers"": [""encore""]
        }";
        ...
        public void A_failing_document_leaves_every_documents_assets_unwritten()
        {
            ...
            Write("root.json", RootJson.Replace(@"""modifiers"": [",
                @"""modifiers"": [ { ""id"": ""m"", ""appliesWhen"": { ""type"": ""Nope"" } },"));
        ...
        // A SerializeReference rid is regenerated by the serializer on every
        // write, so it is the one line the importer does not control. The row
        // tokenizes those values and compares everything else byte for byte.
        [Test]
        public void A_second_import_of_identical_json_changes_nothing()
        {
            ...
            var before = Directory.GetFiles(ObjectRoot, "*.asset", SearchOption.AllDirectories)
                .OrderBy(p => p).Select(File.ReadAllText).Select(WithoutRids).ToArray();
        ...
        private static string WithoutRids(string text) =>
            Regex.Replace(text, @"rid: -?\d+", "rid: <regenerated>");
        ...
        public void Sections_on_a_tier_or_on_the_root_abort()
        {
            ...
            Write("root.json", RootJson.Replace(
                @"""declaredTags"": [""income""]",
                @"""declaredTags"": [""income""], ""sections"": [{ ""title"": ""T"", ""visibleWhen"": { ""type"": ""Always"" }, ""scopeId"": ""root"" }]"));
            Write("ch1.json", ChapterJson);
        ...
        {
            Write("root.json", RootJson.Replace(@"""declaredTags"": [""income""]",
                @"""declaredTags"": [""income""], ""children"": [{ ""type"": ""ChapterDefinition"", ""id"": ""ch1"" }]"));

            var thrown = Assert.Throws<ContentImportException>(Import);
            StringAssert.Contains("roster", thrown.Message);
        }

        // Entitlements are root's alone (12.3): the key is real on every scope
        // block, so a chapter authoring one names itself in the refusal rather
        // than reading as a misspelling - the same shape the sections and
        // children refusals above take.
        [Test]
        public void A_chapter_authoring_entitlements_aborts()
        {
            Write("root.json", RootJson);
            Write("ch1.json", ChapterJson.Replace(@"""id"": ""ch1"",",
                @"""id"": ""ch1"", ""entitlements"": [""side_pass""],"));

            var thrown = Assert.Throws<ContentImportException>(Import);
            StringAssert.Contains("scope 'ch1' authors entitlements", thrown.Message);
        }
