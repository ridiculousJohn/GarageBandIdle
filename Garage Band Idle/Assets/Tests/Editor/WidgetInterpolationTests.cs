...
namespace RidiculousGaming.GarageBandIdle.Tests
{
    ...
    public class WidgetInterpolationTests
    {
        ...
        public void ADrainingPoolInterpolatesDownAndStopsAtZero()
        {
            ...
            widget.Refresh();
            Assert.AreEqual("8.5", value.text);

            fx.Clock.Frame(fx.At(1), 1);
            widget.Interpolate();
            Assert.AreEqual("7", value.text);

            fx.Clock.Frame(fx.At(10), 9);
            widget.Interpolate();
            Assert.AreEqual("0", value.text, "ten seconds of that slope would read -6.5");
        }
        ...
        public void ACompletedBarNeverDrawsPastItsFillAmount()
        {
            ...
            Assert.AreEqual("100 / 100", progress.text);
            AssertClose(100, fill.value);

            fx.Clock.Frame(fx.At(2), 1);
            widget.Interpolate();
            Assert.AreEqual("100 / 100", progress.text, "a second of the 2/s slope changes nothing");
            AssertClose(100, fill.value);
        }
        ...
        public void TheFrameThatRefreshedShowsTruthExactly()
        {
            ...
            var value = widget.Root.Q<Label>("value");
            widget.Refresh();
            Assert.AreEqual("5", value.text, "ten seconds at 0.5/s");

            widget.Interpolate();
            Assert.AreEqual("5", value.text);
        }

        // A non-tick transaction clears the report, and the display then sits at
        // truth until the next tick measures the new state: a command can
        // invalidate every slope the last tick measured.
        [Test]
        public void ACommandClearsTheSlopeAndTheDisplayStopsMoving()
        {
            var fx = new Fixture();
            fx.Tree.Tier1.generatorCounts["practice_amp"] = 1;
            fx.Session.Tick(10, fx.At(10));
            Assert.IsNotNull(fx.Session.LastTick, "the tick measured a slope");

            Assert.IsTrue(fx.Session.FireProducer(fx.Ctx(10), fx.Tree.TapProducer), "the tap fired");
            Assert.IsNull(fx.Session.LastTick, "the command cleared it");

            var widget = fx.Widget("currency_line", "CurrencyLine.uxml", fx.Tree.Cash);
            var value = widget.Root.Q<Label>("value");
            widget.Refresh();
            Assert.AreEqual("6", value.text, "five from the tick plus the tap's yield");

            fx.Clock.Frame(fx.At(14), 4);
            widget.Interpolate();
            Assert.AreEqual("6", value.text, "no slope, so four seconds move nothing");
        }
    }
}
