...
namespace RidiculousGaming.GarageBandIdle.Tests
{
    public class SaveSystemTests
    {
        ...
        {
            tree = new TestTree();
            return SaveSystem.TryDeserialize(json, tree.Content, out root);
        }
        ...
        private LoadOutcome LoadDisk(out RootScopeState root)
        {
            ...
            return SaveSystem.LoadFromDisk(SavePath, tree.Content, out root);
        }
        ...
        private static TestTree Populate(TestTree tree)
        {
            ...
            // A timed record is a TIMER's value, so a legal one names a timer the
            // holding scope's chain declares. The filter runs on LOAD against
            // the tree being loaded into, so the row reading this back declares
            // the timer there as well.
            tree.RootDef.declaredTimers.Add("encore_timer");
            tree.Root.timedBuffs.Add(new TimedBuff { buffId = "encore_timer", expiresAtUtc = new DateTime(2026, 8, 19, 0, 0, 0, DateTimeKind.Utc) });
        ...
        public void Loading_without_a_content_tree_throws()
        {
            ...
            Assert.Throws<ArgumentNullException>(
                () => SaveSystem.TryDeserialize(json, default, out _));
        }
        ...
        public void Saving_after_backup_recovery_never_rotates_the_corrupt_primary_into_the_backup()
        {
            ...
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);
            tree.Root.balances["records"] = 2;
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);   // backup now holds records=1
            Corrupt(SavePath);

            tree.Root.balances["records"] = 3;
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);   // must NOT install the corrupt file as .bak
        ...
        public void Unreadable_primary_falls_back_to_the_backup()
        {
            ...
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);
            tree.Root.balances["records"] = 2;
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);   // backup = records 1, primary = records 2
        ...
        // fresh one built here rather than the one that was serialized.
        [Test]
        public void A_timed_record_naming_no_timer_on_the_chain_is_dropped()
        {
            var expiry = new DateTime(2026, 8, 19, 0, 0, 0, DateTimeKind.Utc);
            var saved = new TestTree();
            saved.Root.timedBuffs.Add(new TimedBuff { buffId = "encore_timer", expiresAtUtc = expiry });
            saved.Root.timedBuffs.Add(new TimedBuff { buffId = "ghost_buff", expiresAtUtc = expiry });
            var json = SaveSystem.Serialize(saved.Root);

            var loaded = new TestTree();
            loaded.RootDef.declaredTimers.Add("encore_timer");

            LogAssert.Expect(LogType.Warning, new System.Text.RegularExpressions.Regex("timed buff 'ghost_buff'"));
            Assert.IsTrue(SaveSystem.TryDeserialize(json, loaded.Content, out var root));

            Assert.AreEqual(1, root.timedBuffs.Count);
            Assert.AreEqual("encore_timer", root.timedBuffs[0].buffId);
        ...
        [Test]
        public void An_expired_record_for_a_declared_timer_survives_the_load()
        {
        ...
        public void Newer_schema_primary_never_rotates_into_the_backup()
        {
            ...
            tree.Root.balances["records"] = 3;
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);   // must NOT install the newer-schema file as .bak
        ...
        {
            var tree = new TestTree();
            SaveSystem.WriteAtomic(SavePath, tree.Root, tree.Content);   // primary exists, no backup yet
