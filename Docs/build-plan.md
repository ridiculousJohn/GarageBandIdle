...
| 8 | Chapter 1 JSON + importer + walkthrough tests | **slice B landed 2026-08-28** - 417/417 green: `chapter-01.json` authored from the content doc (sections 3-10), the ch1 subtree written to `Assets/ScriptableObjects/ch1`, its chapter root carrying the `chapter` label, and `Chapter1ContentTests` - the import reports zero validator ERRORS and the two whitelisted `FlagNoSetter` warnings on the story latches. Slice A (2026-08-27, 405/405) brought `ComposedContent` and the load-and-compose `ContentDatabase`, the save reshaped onto the pair, the document-scoped importer with its lints and preflight-the-union contract, `root.json`, and the root entry in one PackTogether group; slices C-D remain |
