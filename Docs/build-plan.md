...
| - | Load-time linking | **not started** - `load-linking-plan.md`: static wiring resolved once when a tree is built, read by reference afterward. Changeset 0: a link pass at the end of `ScopeState.Build` resolves every scope reference (`ResetScope`, `RestartScope`, `ExecuteRung`, section and module scopes) to its runtime node, `FindInSubtree` deleted; the two event conditions lose `host` and read outward (12.4), the reset refuses over an armed reward on its own (12.5), the chapter capstone's guard leg leaves the content; `appliesWhen` judged at the applied node. Changeset 1: the same pass compiles the gather - per-source effect links, currency-stage links, the chapter's contributor and bar lists - so no tick walks a subtree, walks a chain, or string-matches an effect |
