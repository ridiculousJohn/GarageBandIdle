...
- **Next: slice 5.7, THEN slice 6.** 5.7 is written in the doc (`cb204ff`) but NOT built: it retires `FanSystem`, making fan accrual a module-held production config plus a derived `FanRate` modifier, and it must land before 6 because the release walks each system's facts. It also deletes `FansConfig.ActiveWhen`, which 5.6 just added - that is a relocation onto the config's `gate`, not churn, and the doc says so. See [[project-layout-and-workflow]] for the slice workflow and [[unity-headless-verify-loop]] for verification.
