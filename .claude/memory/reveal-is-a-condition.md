...
metadata: 
  ...
  modified: 2026-08-07T22:07:43.047Z
---
...
- **Slice 6 (`f7ad34f`) carried reveal further than this slice did:** sections hold no state at all, generator rows derive from their unlock condition live, and flags carry declared lifetimes - so a release can take a reveal back, which a latch could not. 5.7 landed (`1227bab`) and retired `FanSystem` - see [[fan-accrual-is-production]]. It deleted `FansConfig.ActiveWhen`, which 5.6 had just added: that is a relocation onto the production config's `gate`, not churn, and it was only a relocation BECAUSE 5.6 had already made it a Condition. The bar group's `VisibleWhen` from this slice is untouched and still live. See [[project-layout-and-workflow]] for the slice workflow and how to derive the current position, and [[unity-headless-verify-loop]] for verification.
