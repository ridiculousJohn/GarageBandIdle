...
- **One guarantee is still test-only, never yet exercised in play.** Focus switching with more than one context: `SetFocus` has only ever seen one, so "exactly one focused" and the `Unfocus` timestamp are covered by `EconomyContextTests` alone, and slice 8 is the first thing that makes it real. Do not treat it as field-proven. The other has since been exercised: `EconomyContext.ProjectModifiers()` after construction had only ever run on an empty store, because the release did not exist until slice 6 (`f7ad34f`) became the first production caller to re-project a non-empty one.
...
- Both consolidations after 5.5 landed before slice 6: 5.6 retired the last bare `revealFlag` fields ([[reveal-is-a-condition]]) and 5.7 retired `FanSystem` ([[fan-accrual-is-production]]). See [[project-layout-and-workflow]] for the slice workflow and how to derive the current position, and [[unity-headless-verify-loop]] for verification.
