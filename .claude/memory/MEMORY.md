...
- [Economy context as built](economy-context-as-built.md) — slice 5.5 landed `f12ba3e`; re-projection is the only door a modifier enters; which guarantees are test-only (rebuild-after-construction, multi-context focus); next is 5.7 before slice 6
- [Reveal is a Condition](reveal-is-a-condition.md) — slice 5.6 landed `5b8a917`; the fail-open gap it left, the two stale-key refusal shapes, why the fan base rate never checks for a band, and that 5.7 is written but unbuilt
