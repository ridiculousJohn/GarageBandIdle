...
- [Unity headless verify loop](unity-headless-verify-loop.md) — batchmode import+tests when editor closed (check UnityLockfile first, one level down); exit code 0 proves nothing, grep for `error CS`; 274 tests at `f7ad34f`
...
- [Economy context as built](economy-context-as-built.md) — slice 5.5 landed `f12ba3e`; re-projection is the only door a modifier enters; which guarantees are test-only (rebuild-after-construction, multi-context focus); next is slice 7
