...
- Tests: same as the import line but swap `-executeMethod ...` for `-runTests -testPlatform EditMode -testResults <xml>` and **DROP `-quit`** - the test runner exits on its own, and `-quit` tears the editor down before it ever starts. Check `total=.. passed=.. failed="0"` on the first summary line. **274 at `f7ad34f`** (2026-08-06, slice 6 plus the generator-latch and earned-total fixes). 240 at `7f9c259`, 211 at `1227bab`, 202 at `5b8a917`, 201 at `f12ba3e`, 187 at `dfded84`, 60 at slice 3.5. The number moves every slice - re-derive from a clean run and update this line rather than trusting it.
