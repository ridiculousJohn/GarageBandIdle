...
- Tests: same but `-runTests -testPlatform EditMode -testResults <xml>`; check `total=.. passed=.. failed="0"` on the first summary line. **201 as of `f12ba3e`** (2026-08-04, slice 5.5 economy context; 187 at `dfded84`, 60 at slice 3.5). The number moves every slice - re-derive from a clean run and update this line rather than trusting it.
