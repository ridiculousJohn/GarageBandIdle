...
**Dead files still on disk (inventory verified 2026-08-18). Deletion is PENDING John's explicit call — do not delete unprompted:**
- `Docs/claude-code-build-prompts.md` — dead, never read it.
- `Docs/chapter-01-garage.json` — dead schema, but a candidate reference when authoring Chapter 1's numbers (old generator names, costs, curves).
- `Garage Band Idle/Assets/Scripts/**` (106 .cs files) and `Garage Band Idle/Assets/Tests/Editor/**` (20 .cs files) — the rejected architecture and its abandoned test suite.
- Architecture-neutral salvage candidates John was shown: `BigNumber.cs` (break_infinity wrapper — the new §12.13 layout wants one), `NumberFormatter.cs`, the `SingletonManager` utility, and the editor drawers `SubclassPickerDrawer.cs` / `DefinitionIdDrawer.cs` (the `[DefinitionId]` dropdown machinery).
- Options on the table: delete everything; delete everything except the utilities and the chapter JSON; or leave it all until the new build cannibalizes. Everything is in git history regardless, so nothing is permanently lost by deleting.
