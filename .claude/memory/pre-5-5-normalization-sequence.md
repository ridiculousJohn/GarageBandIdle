...
**CONFIRMED 2026-08-04, was open across two sessions:** the `[ModifierQualifierId]` qualifier dropdown in `ModifierQualifierIdDrawer` renders correctly in the inspector - John checked it and reported it working. Nothing outstanding here.

The three assets carrying a non-empty qualifier list are the check sites, all under `Assets/ScriptableObjects/Upgrades/`: `amp_strings` and `kit_upgrade` (`GeneratorOutput`, so a generator-id popup) and `tight_set` (`CurrencyProduction`, a currency-id popup). `stage_presence` is the negative control - `TapValue` takes no qualifier, so the drawer is SUPPOSED to render a plain string field there. That asymmetry is the whole reason this needs an eyeball: the drawer fails SOFT to a plain string field, which is indistinguishable from correct behavior on a global target. It cannot be verified by [[unity-headless-verify-loop]] (a headless run never opens an inspector) and only its `SiblingPath` string arithmetic has tests, while the shape the real assets produce is the `[SerializeReference]` path (`references.RefIds.Array.data[N].data._qualifiers.Array.data[M]`). Re-check the same four assets if the drawer or `ModifierTargetKey.QualifierDefinitionType` is touched again.
